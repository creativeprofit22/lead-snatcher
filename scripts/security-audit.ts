...
function checkFile(filePath: string, content: string, lines: string[]) {
  ...
  const secretPatterns = [
    ...
    {
      pattern: /password\s*[:=]\s*['"][^'"]{8,}['"](?!.*process\.env)/i,
  ...
  if (filePath.includes('/api/') && filePath.endsWith('route.ts')) {
    const hasAuth =
      ...
      content.includes('requireRouteUserId') ||
      content.includes('requireRouteValidUser') ||
  ...
  if (filePath.includes('/api/') && content.includes('request.json()')) {
    if (!content.includes('try')) {
      ...
      if (lastTry === -1 || lastCatch > lastTry) {
        addIssue('MEDIUM', relPath, 'request.json() without try-catch');
      }
    }
  }
  ...
  if (content.includes('searchParams.get(')) {
    ...
    const hasSortValidation =
      content.includes('ALLOWED_SORT_FIELDS') ||
      ...
      lines.forEach((line, idx) => {
        ...
        if (line.includes('orderBy') && line.includes('[') && line.includes('sortBy')) {
          addIssue(
            'HIGH',
            relPath,
            'Possible injection via orderBy parameter - add whitelist validation',
            idx + 1
          );
        }
      });
...
function checkPrismaSchema() {
  ...
  if (cascadeMatches && cascadeMatches.length > 5) {
    addIssue(
      ...
      `${cascadeMatches.length} cascade deletes found - review for unintended data loss`
    );
  }
...
async function runAudit() {
  ...
  if (high.length > 0) {
    console.log(
      `${YELLOW}${BOLD}⚠️  HIGH priority issues found - review before publishing${RESET}\n`
    );
    return 1;
  }
